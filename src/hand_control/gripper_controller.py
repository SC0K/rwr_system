...
class MuscleGroup:
    ...
    """
    attributes = ["joint_ids", "tendon_ids",
                  "motor_ids", "motor_map", "spool_rad"]
...
class GripperController:
    ...
    def __init__(self, port: str = '/dev/ttyUSB0', config_yml: str = "gripper_defs.yaml", calibration: bool = False, maxCurrent: int = 150):
        ...
        self._load_musclegroup_yaml(os.path.join(
            os.path.dirname(os.path.abspath(__file__)), config_yml))
    ...
    def _load_musclegroup_yaml(self, filename):
        ...
        for muscle_group_name, muscle_group_data in data['muscle_groups'].items():
            self.muscle_groups.append(MuscleGroup(
                muscle_group_name, muscle_group_data))
    ...
    def tendon_pos2motor_pos(self, tendon_lengths):
        ...
        for muscle_group in self.muscle_groups:
            ...
            for m_i in range(m_nr):
                ...
                motor_pos[m_idx + m_i] = tendon_lengths[t_idx+t_i] / \
                    muscle_group.spool_rad[t_i]
            m_idx += m_nr
            t_idx += t_nr
        return motor_pos
    ...
    def motor_pos2tendon_pos(self, motor_pos):
        ...
        for muscle_group in self.muscle_groups:
            ...
            for m_i in range(m_nr):
                ...
                for i in t_i:
                    tendon_lengths[t_idx+i] = motor_pos[m_idx +
                                                        m_i]*muscle_group.spool_rad[i]
            m_idx += m_nr
            t_idx += t_nr
    ...
    def pose2motors(self, joint_angles):
        ...
        for muscle_group in self.muscle_groups:
            ...
            if muscle_group.name == "wrist":
                tendon_lengths[t_idx:t_idx+t_nr] = fk.pose2tendon_wrist(
                    joint_angles[j_idx])
            elif muscle_group.name == "thumb":
                tendon_lengths[t_idx:t_idx+t_nr] = fk.pose2tendon_thumb(
                    joint_angles[j_idx], joint_angles[j_idx+1], joint_angles[j_idx+2], joint_angles[j_idx+3])
            else:
                # pass # tendon_lengths[t_idx:t_idx+t_nr] = fk.pose2tendon_finger2(joint_angles[j_idx],joint_angles[j_idx+1])
                tendon_lengths[t_idx:t_idx+t_nr] = fk.pose2tendon_finger(
                    joint_angles[j_idx], joint_angles[j_idx+1], joint_angles[j_idx+2])
    ...
    def init_joints(self, calibrate: bool = False, maxCurrent: int = 150):
        ...
        if not calibrate and cal_exists:
            ...
            self.write_desired_motor_current(
                100 * np.ones(len(self.motor_ids)))
            time.sleep(0.2)
    ...
    def write_desired_joint_angles(self, joint_angles: np.array):
        ...
        """
        motor_pos_des = self.pose2motors(np.deg2rad(
            joint_angles)) - self.motor_pos_norm + self.motor_id2init_pos
        # print(motor_pos_des)
